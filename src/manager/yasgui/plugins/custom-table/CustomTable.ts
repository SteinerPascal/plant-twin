...
/**
 *  This is copier from YASR/plugins/table. It is slightly manipulated to change the URIs to react internal links
 * 
...
export default class CustomTable implements Plugin<PluginConfig> {
  ...
  constructor(yasr: Yasr) {
    ...
    this.config = CustomTable.defaults;
  }
  ...
  private getUriLinkFromBinding(binding: Parser.BindingValue, prefixes?: { [key: string]: string }) {
    ...
    return `${hideBrackets ? "" : "&lt;"}<a class='iri' style="cursor: pointer; color:blue;">${visibleString}</a>${
      hideBrackets ? "" : "&gt;"
    }`;
  }
  ...
  public draw(persistentConfig: PersistentConfig) {
    ...
    this.setURIClickListener(this.tableEl)
  }

  // Inserted method to handle links from the bindings to  
  private setURIClickListener(tableEl: HTMLTableElement) {
